...
export interface JornalOptions {
  headerPath?: string;
...
export class CardGenerator extends EventEmitter {
  ...
  async generateCards(excelFilePath: string, originalFileName?: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      archive.pipe(output);
      fs.readdirSync(OUTPUT_DIR).forEach(f => { 
        if (f.endsWith(".pdf")) archive.file(path.join(OUTPUT_DIR, f), { name: f }); 
      });
      archive.finalize();
    });
  }

  async generateJornal(excelFilePath: string, options: JornalOptions = {}): Promise<string> {
    await this.initialize();
    ...
    const bg = options.backgroundColor || "#064e3b";
    const catBg = options.categoryBoxColor || "#1d4ed8";

    let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
      * { margin: 0; padding: 0; box-sizing: border-box; -webkit-print-color-adjust: exact; }
      body { background: ${bg}; width: ${totalW + 100}px; padding: 60px 0; font-family: Arial, sans-serif; display: flex; flex-direction: column; align-items: center; }
      .container { width: ${totalW}px; }
      .header-img { width: 100%; margin-bottom: 40px; border-radius: 20px; }
      .cat-label { background: ${catBg}; color: white; width: 100%; height: 120px; line-height: 120px; font-size: 60px; font-weight: bold; text-align: center; border-radius: 20px; margin: 50px 0 30px 0; text-transform: uppercase; }
      ...
      .footer { width: 100%; color: white; text-align: center; font-size: 24px; margin-top: 60px; padding: 40px; opacity: 0.8; }
    </style></head><body><div class="container">`;

    if (options.headerPath) {
      const headerBase64 = this.imageToBase64(options.headerPath);
      if (headerBase64) html += `<img src="${headerBase64}" class="header-img">`;
    }
    ...
    if (options.footerText) {
      html += `<div class="footer">${options.footerText}</div>`;
    }
