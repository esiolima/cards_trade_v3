  ...
  async generateJornalPreview(options: any): Promise<string> {
    ...
    let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
      * { margin: 0; padding: 0; box-sizing: border-box; -webkit-print-color-adjust: exact; }
      ...
      /* ESTILOS DOS CARDS (CONSOLIDADOS) */
      .card { width: 700px; height: 1058px; border: 8px solid #e0b84b; border-radius: 40px; background: #ffffff; padding: 60px 40px 40px 40px; display: flex; flex-direction: column; justify-content: space-between; position: relative; color: #1F3C6E; text-transform: uppercase; }
      .selo { position: absolute; top: -8px; left: -8px; width: 130px; height: 65px; z-index: 999; }
      .selo img { width: 100%; height: 100%; object-fit: contain; }
      .topo { display: flex; flex-direction: column; align-items: center; }
      .logo { margin-bottom: 20px; }
      .logo img { max-width: 240px; max-height: 120px; }
      .tarja { background: #1a7d00; color: #ffffff; font-weight: 700; font-size: 42px; text-align: center; padding: 18px 40px; border-radius: 999px; margin-bottom: 30px; white-space: nowrap; }
      .area-central { display: flex; flex-direction: column; align-items: center; gap: 25px; }
      .texto-topo { font-size: 26px; line-height: 1.2; text-align: center; }
      .valor-container { display: flex; justify-content: center; align-items: flex-end; }
      .valor-texto { font-weight: 900; font-size: 180px; line-height: 1; }
      .desconto { font-size: 44px; font-weight: 700; }
      .complemento { font-size: 24px; line-height: 1.2; text-align: center; }
      .legal { font-size: 18px; line-height: 1.2; text-align: center; }
      .rodape { text-align: center; }
      .uf { font-size: 22px; font-weight: 700; }
      .segmento-wrapper { margin-top: 12px; font-size: 18px; }
      .footer { width: ${totalW}px; margin-top: 100px; padding: 60px; color: white; font-size: 40px; text-align: center; }
    ...
    for (const [category, items] of Object.entries(groupedRows)) {
      ...
      for (const item of items) {
        ...
        cHtml = cHtml.replaceAll("{{TEXTO}}", String(item.texto ?? ""))
                     ...
                     .replaceAll("{{LOGO}}", logoBase64)
                     .replaceAll("{{SELO}}", seloBase64)
                     // LIMPEZA FINAL: Remove tudo que não é conteúdo do card, incluindo scripts
                     .replace(/<!DOCTYPE[^>]*>/gi, "")
        ...
        html += `<td class="card-item"><div class="inner-card-wrapper">${cHtml.trim()}</div></td>`;
        count++;
      }
